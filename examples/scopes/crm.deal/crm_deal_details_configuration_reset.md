...
## Description
Resets deal details card configuration.
...
## Notes
`crm.deal.details.configuration.reset` is not list-capable in SDK usage, so `.as_list()` and `.as_list_fast()` examples are not applicable.
