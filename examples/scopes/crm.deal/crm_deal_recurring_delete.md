...
## Description
Deletes recurring deal template settings.
...
## Notes
`crm.deal.recurring.delete` is not list-capable in SDK usage, so `.as_list()` and `.as_list_fast()` examples are not applicable.
