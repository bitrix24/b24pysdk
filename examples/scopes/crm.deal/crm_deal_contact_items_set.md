...
## Description
Sets the full set of contacts linked to a deal.
...
## Notes
`crm.deal.contact.items.set` is not list-capable in SDK usage, so `.as_list()` and `.as_list_fast()` examples are not applicable.
