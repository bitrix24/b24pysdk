...
## Description
Deletes all contact bindings from a deal.
...
## Notes
`crm.deal.contact.items.delete` is not list-capable in SDK usage, so `.as_list()` and `.as_list_fast()` examples are not applicable.
