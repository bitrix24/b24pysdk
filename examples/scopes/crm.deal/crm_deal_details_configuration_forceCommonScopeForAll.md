...
## Description
Forces common deal card configuration for all users.
...
## Notes
`crm.deal.details.configuration.forceCommonScopeForAll` is not list-capable in SDK usage, so `.as_list()` and `.as_list_fast()` examples are not applicable.
