...
from b24pysdk.api.responses import B24AppInfoInstall
...
def test_from_dict_variants(json_data: JSONDict):
    obj = B24AppInfoInstall.from_dict(json_data)
...
def test_frozen_instance():
    assert_frozen_instance(B24AppInfoInstall, _JSON_DATA_FULL_INSTALL, "installed")


def test_equality_disabled():
    assert_equality_disabled(B24AppInfoInstall, _JSON_DATA_FULL_INSTALL)


def test_is_dataclass():
    assert_is_dataclass(B24AppInfoInstall)


def test_slots_defined_conditionally_by_python_version():
    assert_slots(B24AppInfoInstall)
