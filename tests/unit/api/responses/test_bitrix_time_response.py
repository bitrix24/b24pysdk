...
from tests.unit.examples import EXAMPLE_TIME_1
from tests.unit.helpers import assert_equality_disabled, assert_frozen_instance, assert_is_dataclass, assert_slots

pytestmark = [
    pytest.mark.unit,
    pytest.mark.responses,
    pytest.mark.bitrix_time_response,
]
...
def test_from_dict_variants(json_data: JSONDict, expected_operating_reset_at: Optional[datetime], expected_operating: Optional[float]):
    obj = BitrixTimeResponse.from_dict(json_data)
...
def test_frozen_instance():
    assert_frozen_instance(BitrixTimeResponse, EXAMPLE_TIME_1, "start")


def test_equality_disabled():
    assert_equality_disabled(BitrixTimeResponse, EXAMPLE_TIME_1)


def test_is_dataclass():
    assert_is_dataclass(BitrixTimeResponse)


def test_slots_defined_conditionally_by_python_version():
    assert_slots(BitrixTimeResponse)
