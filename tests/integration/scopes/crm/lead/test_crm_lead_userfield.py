...
_SETTINGS_DEFAULT_VALUE: Text = f"{SDK_NAME} LEAD UF DEFAULT"
...
@pytest.mark.dependency(name="test_crm_lead_userfield_add")
def test_crm_lead_userfield_add(bitrix_client: BaseClient, cache: Cache):
    """"""

    userfield_name = f"UF_CRM_{SDK_NAME.upper()}_{int(Config().get_local_datetime().timestamp() * (10 ** 6))}"

    bitrix_response = bitrix_client.crm.lead.userfield.add(
        fields={
    ...
    cache.set("lead_userfield_id", userfield_id)
    cache.set("lead_userfield_name", userfield_name)


@pytest.mark.dependency(name="test_crm_lead_userfield_update", depends=["test_crm_lead_userfield_add"])
def test_crm_lead_userfield_update(bitrix_client: BaseClient, cache: Cache):
    """"""

    userfield_id = cache.get("lead_userfield_id", None)
    assert isinstance(userfield_id, int), "Userfield ID should be cached"

    bitrix_response = bitrix_client.crm.lead.userfield.update(
...
@pytest.mark.dependency(name="test_crm_lead_userfield_get", depends=["test_crm_lead_userfield_update"])
def test_crm_lead_userfield_get(bitrix_client: BaseClient, cache: Cache):
    """"""

    userfield_id = cache.get("lead_userfield_id", None)
    assert isinstance(userfield_id, int), "Userfield ID should be cached"

    userfield_name = cache.get("lead_userfield_name", None)
    assert isinstance(userfield_name, str), "Userfield name should be cached"

    bitrix_response = bitrix_client.crm.lead.userfield.get(
        bitrix_id=userfield_id,
    ).response
...
@pytest.mark.dependency(name="test_crm_lead_userfield_list", depends=["test_crm_lead_userfield_get"])
def test_crm_lead_userfield_list(bitrix_client: BaseClient, cache: Cache):
    """"""
    ...
    bitrix_response = bitrix_client.crm.lead.userfield.list(
        filter={
            "ID": userfield_id,
        },
    ).response
...
@pytest.mark.dependency(name="test_crm_lead_userfield_list_as_list", depends=["test_crm_lead_userfield_list"])
def test_crm_lead_userfield_list_as_list(bitrix_client: BaseClient):
    """"""

    bitrix_response = bitrix_client.crm.lead.userfield.list().as_list().response
...
@pytest.mark.dependency(name="test_crm_lead_userfield_list_as_list_fast", depends=["test_crm_lead_userfield_list_as_list"])
def test_crm_lead_userfield_list_as_list_fast(bitrix_client: BaseClient):
    """"""

    bitrix_response = bitrix_client.crm.lead.userfield.list().as_list_fast(descending=True).response
...
@pytest.mark.dependency(name="test_crm_lead_userfield_delete", depends=["test_crm_lead_userfield_list_as_list_fast"])
def test_crm_lead_userfield_delete(bitrix_client: BaseClient, cache: Cache):
    """"""

    userfield_id = cache.get("lead_userfield_id", None)
    assert isinstance(userfield_id, int), "Userfield ID should be cached"

    bitrix_response = bitrix_client.crm.lead.userfield.delete(
        bitrix_id=userfield_id,
    ).response
