...
_SETTINGS_DEFAULT_VALUE: Text = f"{SDK_NAME} COMPANY UF DEFAULT"
...
@pytest.mark.dependency(name="test_crm_company_userfield_add")
def test_crm_company_userfield_add(bitrix_client: BaseClient, cache: Cache):
    """"""

    userfield_name = f"UF_CRM_{SDK_NAME.upper()}_{int(Config().get_local_datetime().timestamp() * (10 ** 6))}"

    bitrix_response = bitrix_client.crm.company.userfield.add(
        fields={
    ...
    cache.set("company_userfield_id", userfield_id)
    cache.set("company_userfield_name", userfield_name)


@pytest.mark.dependency(name="test_crm_company_userfield_update", depends=["test_crm_company_userfield_add"])
def test_crm_company_userfield_update(bitrix_client: BaseClient, cache: Cache):
    """"""

    userfield_id = cache.get("company_userfield_id", None)
    assert isinstance(userfield_id, int), "Userfield ID should be cached"

    bitrix_response = bitrix_client.crm.company.userfield.update(
...
def test_crm_company_userfield_list(bitrix_client: BaseClient, cache: Cache):
    ...
    bitrix_response = bitrix_client.crm.company.userfield.list(
        filter={
            "ID": userfield_id,
        },
    ).response
...
def test_crm_company_userfield_list_as_list(bitrix_client: BaseClient):
    ...
    bitrix_response = bitrix_client.crm.company.userfield.list().as_list().response
...
def test_crm_company_userfield_list_as_list_fast(bitrix_client: BaseClient):
    ...
    bitrix_response = bitrix_client.crm.company.userfield.list().as_list_fast(descending=True).response
...
@pytest.mark.dependency(name="test_crm_company_userfield_delete", depends=["test_crm_company_userfield_list_as_list_fast"])
def test_crm_company_userfield_delete(bitrix_client: BaseClient, cache: Cache):
    """"""

    userfield_id = cache.get("company_userfield_id", None)
    assert isinstance(userfield_id, int), "Userfield ID should be cached"

    bitrix_response = bitrix_client.crm.company.userfield.delete(
        bitrix_id=userfield_id,
    ).response
