...
pytestmark = [
    ...
    pytest.mark.imconnector_update,
]
_MESSAGES = [
    {
        "user": {"id": "sdk-user-1", "name": "SDK User", "last_name": "Test"},
        "message": {"id": "sdk-message-1", "date": int(time.time()), "text": "SDK Updated"},
...
def test_messages(bitrix_client: BaseClient):
    ...
    try:
        ...
        bitrix_response = bitrix_client.imconnector.update.messages(
            connector=connector_id,
            line=line_id,
            messages=_MESSAGES,
        ).response
        ...
        assert isinstance(bitrix_response.result, (bool, dict)), "imconnector.update.messages result should be bool or dict"
    finally:
